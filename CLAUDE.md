...
Ruby is the original; Python, Lua, and JavaScript are faithful ports, with the same field layout, the same monotonicity contract, and the same demo output. No gemspec, Gemfile, `pyproject.toml`, rockspec, `package.json`, or test framework.
...
Self-tests live at the bottom of each file: `if __FILE__ == $PROGRAM_NAME` (Ruby), `if __name__ == "__main__"` (Python), `if modname == nil` (Lua, `modname` being the `...` captured at the top), `require.main === module` (JavaScript). There is no separate test file; new behavior gets a new section there.

**Results print as `true`/`false` or `✓`/`✗` and never set a non-zero exit code, so read the output.**

## Architecture

Three layers in each file. Ruby namespaces them in a `UUIDv7` module, Python in the module itself, Lua in a returned table `M` (`local uuid_v7 = require("uuid_v7")`), JavaScript in `module.exports`.

- **`Generator`**: stateful, holding `last_ms` and `seq`, advanced by `next_state`. Bit-packing (`assemble`) is pure. Ruby and Python lock the state; Lua and JavaScript do not (see parity table).
...
Ruby and Python pack this into one arbitrary-precision integer formatted with `%032x`; `decode` reverses it with shifts and masks. JavaScript does the same in BigInt, since a Number is exact only to 2^53.

Lua's integers are 64-bit and cannot, so `assemble` emits hex group by hex group. Groups align with field boundaries except `rand_b`, whose top 14 bits share group 4 with the variant; `decode` rebuilds it as `((group4 & 0x3FFF) << 48) | group5`. This is the only structural divergence among the four; JavaScript packs a single value like Ruby and Python, just in BigInt.
...
|---|---|---|---|---|
| Namespace | `UUIDv7` module | the module itself | returned table `M` | `module.exports` (CommonJS) |
| Function names | snake_case | snake_case | snake_case | camelCase (`generateRandom`, `generateBulk`) |
| Method call | `gen.generate` | `gen.generate()` | `gen:generate()` (colon) | `gen.generate()` |
| Predicate | `valid?` | `is_valid` | `is_valid` | `isValid` |
| Invalid input | `ArgumentError` | `ValueError` | `error()`; `is_valid` uses `pcall` | `TypeError` |
| Format check | `UUID_REGEX` | `UUID_REGEX` (`re`) | `UUID_PATTERN`, with 8-4-4-4-12 spelled out because Lua patterns lack `{n}` and alternation | `UUID_REGEX` |
| CSPRNG | `SecureRandom.random_number(n)` | `secrets.randbelow(n)` | `/dev/urandom`, falling back to `math.random` | `crypto.getRandomValues`, no fallback |
| Clock | `Process.clock_gettime(CLOCK_REALTIME, :millisecond)` | `time.time_ns() // 1_000_000` | luaposix / luasocket if present, else `os.time` + `os.clock` | `Date.now()` |
| Concurrency | `Mutex` | `threading.Lock` | none, since standard Lua has no preemptive threads | none, since there is one event loop |
| Packing | one 128-bit integer | one 128-bit integer | per hex group (64-bit ceiling) | one 128-bit `BigInt` |
| `decode` timestamp | `Time` (UTC) | `datetime` (tz-aware UTC) | `string`, `"YYYY-MM-DD HH:MM:SS.mmm UTC"` | `Date` |
| `decode` numerics | Integer | `int` | 64-bit integer | Number, except `rand_b`, a `BigInt` |
| `decode` result | `Hash`, Symbol keys | `dict`, `str` keys | `table` with `str` keys | plain object with `str` keys |
...
- Bulk type checks: Python rejects `bool` explicitly (`True` is an `int`); Lua's `math.type(n) ~= "integer"` rejects floats like `3.0`; JavaScript's `Number.isInteger` accepts `3.0`, since it has no distinct integer type.
...
- Lua's concurrency test uses four round-robin coroutines and JavaScript's four async tasks awaiting between calls. Both exercise interleaved access to the shared generator (the strongest concurrency each language offers), but neither is evidence of preemptive thread safety.
...
- Doc-comment style is per-language, never mixed: YARD (`@param`/`@return`/`@raise`) in Ruby; Sphinx docstrings (`:param:`/`:return:`/`:raises:`) plus type hints in Python; LDoc `--` comments in Lua; JSDoc (`@param`/`@returns`/`@throws`) in JavaScript.
...
- A workaround forced by a language limit documents *what the limit is*, not just what the code does. Lua's `assemble`, `current_ms`, and `rand_bits` carry longer comments for that reason, so don't trim them.
